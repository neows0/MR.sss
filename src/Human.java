...
public class Human extends GameObject {
  ...
  private int bobbel = 0;
  private boolean bobbelUp = false;
  ...
  public Human(int x, int y, ID id) {
    ...
    if (Game.player != null && Game.player.getEntity() != null){
  ...
  public void render(Graphics g) {
    ...
    int offset = 0;
    slow = !slow;
    if (z == ground && (dY != 0 || dX != 0)){
      offset = bobbel;
      if (!slow){
        if(bobbel == 0 || bobbel == 5)
          bobbelUp = !bobbelUp;
        if (bobbelUp){
          bobbel++;
        }
    ...
    g.drawImage(getImage(), tempX - IMGWIDTH / 2,
    tempY - IMGHEIGHT / 2 - z - offset,
    tempX + IMGWIDTH / 2, tempY + IMGHEIGHT / 2 - z - offset,
